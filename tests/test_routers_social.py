...
@pytest.mark.parametrize("provider", ["google", "facebook", "vk"])
def test_login(provider: str):
    url = app.url_path_for("social:login", provider=provider)
    with mock.patch(
        f"fastapi_auth.routers.social.SocialService.login_{provider}",
...
@pytest.mark.parametrize("provider", ["google", "facebook", "vk"])
@mock.patch(
    "fastapi_auth.routers.social.check_state",
    mock.Mock(return_value=True),
)
def test_callback(provider: str):
    patcher_callback = mock.patch(
        f"fastapi_auth.routers.social.SocialService.callback_{provider}",
        mock.AsyncMock(
            return_value=(
                None,
                None,
    ...
    patcher_resolve_user = mock.patch(
        "fastapi_auth.routers.social.SocialService.resolve_user",
